...
GPIO.setup(CENTER_MOTOR, GPIO.OUT)
GPIO.setup(RIGHT_MOTOR, GPIO.OUT)

# Global variable for vibration control
vibration_states = {"left": False, "center": False, "right": False}

def vibration_worker():
    while True:
        if vibration_states["left"]:
            GPIO.output(LEFT_MOTOR, GPIO.HIGH)
        else:
            GPIO.output(LEFT_MOTOR, GPIO.LOW)

        if vibration_states["center"]:
            GPIO.output(CENTER_MOTOR, GPIO.HIGH)
        else:
            GPIO.output(CENTER_MOTOR, GPIO.LOW)

        if vibration_states["right"]:
            GPIO.output(RIGHT_MOTOR, GPIO.HIGH)
        else:
            GPIO.output(RIGHT_MOTOR, GPIO.LOW)

        sleep(0.05)  # Short sleep to reduce CPU usage

# Start vibration logic in background
...
while True:
    ...
    for idx, (pos, delay, label) in enumerate(positions):
        ...
        # Update vibration feedback based on zone color
        vibration_states["left"] = any(zone_colors[zone] == RED for zone in ["1st", "4th", "7th"])
        vibration_states["center"] = any(zone_colors[zone] == RED for zone in ["2nd", "5th", "8th"])
        vibration_states["right"] = any(zone_colors[zone] == RED for zone in ["3rd", "6th", "9th"])
