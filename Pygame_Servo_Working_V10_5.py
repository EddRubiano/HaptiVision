...
# Mapping zones to motors
zone_motor_map = {
    "1st": "left", "4th": "left", "7th": "left",
    "2nd": "center", "5th": "center", "8th": "center",
    "3rd": "right", "6th": "right", "9th": "right"
}
...
# Feedback control
feedback_patterns = {
    "GREEN": lambda pin, now, state: GPIO.output(pin, GPIO.LOW),
    
    "RED": lambda pin, now, state: GPIO.output(pin, GPIO.HIGH),
    
    "YELLOW": lambda pin, now, state: (
        GPIO.output(pin, GPIO.HIGH if int(now * 10) % 5 < 2 else GPIO.LOW)
    ),
    
    "GREY": lambda pin, now, state: (
        GPIO.output(pin, GPIO.HIGH if int(now * 20) % 2 == 0 else GPIO.LOW)
    )
}
...
def vibration_feedback():
    while True:
        motor_states = {motor: ("GREEN", 0) for motor in motor_pins}
        ...
        for zone, rgb in zone_colors.items():
            ...
            priority = COLOR_PRIORITY[color_name]
            
            if priority > motor_states[motor][1]:
                motor_states[motor] = (color_name, priority)

        now = time()
        for motor, (color_name, _) in motor_states.items():
            pin = motor_pins[motor]
            feedback_patterns[color_name](pin, now, motor)

        sleep(0.02)  # small delay for smooth feedback updates
...
# Zone color status (must be updated from main program)
zone_colors = {
    "1st": (0, 255, 0), "2nd": (0, 255, 0), "3rd": (0, 255, 0),
    "4th": (0, 255, 0), "5th": (0, 255, 0), "6th": (0, 255, 0),
    "7th": (0, 255, 0), "8th": (0, 255, 0), "9th": (0, 255, 0),
    "10th": (0, 255, 0), "11th": (0, 255, 0), "12th": (0, 255, 0),
}
